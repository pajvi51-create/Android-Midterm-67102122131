...
- WebView, Leaflet และ OpenStreetMap สำหรับแผนที่ภายในแอป
...
app/src/main/
├── AndroidManifest.xml
├── assets/leaflet/                 ไฟล์ Leaflet สำหรับแผนที่ภายใน
...
เมื่อแอปเริ่มทำงาน `MainActivity.onCreate()` จะเรียก `setContent` เพื่อวาด `MidtermApp()` ภายใน `MyApplicationTheme` หน้าหลักใช้ `PrimaryTabRow` แบ่งเป็น 3 แท็บ:

- Lifecycle
- แผนที่
- ค่างวด

ตัวแปร `selectedTab` ใช้ `rememberSaveable` จึงจำแท็บที่เลือกได้เมื่อ Activity ถูกสร้างใหม่จากการหมุนจอ จากนั้น `when (selectedTab)` เลือกแสดงหน้าที่สัมพันธ์กับแท็บ
...
โครงงานเชื่อมต่อกับ GitHub Repository และมี Commit แยก Lifecycle พร้อม Message ชัดเจน เช่น

```text
046abb9 feat: implement onCreate lifecycle and midterm screens
90a5860 feat: show Toast in onStart lifecycle
bc7b820 feat: show Toast in onResume lifecycle
6c26920 feat: show Toast in onPause lifecycle
f071b8c feat: show Toast in onStop lifecycle
a0f3a9a feat: show Toast in onDestroy lifecycle
```

จึงมีประวัติอย่างน้อย 6 Commit ตามเกณฑ์ และ Commit ถูก Push ไปยัง GitHub แล้ว
...
CoordinateForm (Stateless) วาด UI ใหม่
```
...
ฟังก์ชัน `validatedCoordinates()` แปลง String เป็น Double ด้วย `toDoubleOrNull()` ซึ่งปลอดภัยกว่าการใช้ `toDouble()` เพราะข้อมูลผิดจะให้ค่า `null` แทนการ Crash
...
## 12. Stateless Composable: `CoordinateForm()`

ฟังก์ชันนี้ไม่มีการประกาศ State ภายใน รับข้อมูลทั้งหมดผ่าน Parameters และส่ง Event กลับผ่าน Callbacks
...
`OutlinedTextField` ช่องแรกเรียก `onLatitudeChange` และช่องที่สองเรียก `onLongitudeChange` ส่วนปุ่มส่ง Event กลับไปให้ `MapScreen()` จัดการ
...
ฟังก์ชัน `EmbeddedMap()` เป็นความสามารถเสริมจากข้อสอบ ใช้ `AndroidView` นำ Android `WebView` มาใช้ใน Compose และโหลด Leaflet จาก assets ภายใน APK
...
เมื่อ `mapLatitude` หรือ `mapLongitude` เปลี่ยน `mapKey` จะเปลี่ยน ส่วน `update` ของ `AndroidView` จะโหลด HTML ใหม่ แผนที่จึง `setView(position, 16)` ไปยังพิกัดใหม่และสร้าง Marker ตรงกลาง
...
Permission `INTERNET` ใช้ดาวน์โหลดภาพ Tile ของ OpenStreetMap เท่านั้น ส่วนไฟล์ Leaflet ถูกเก็บใน APK เพื่อให้ส่วนควบคุมแผนที่โหลดได้เสถียร
...
- ปุ่มเปิดแผนที่ภายนอกเปิด MapsActivity ได้
...
5. เปิด GitHub และแสดง Commit แยกอย่างน้อย 6 รายการ
